...
def create_combined_clip(folder_path, start_index, end_index, duration):
    ...
    pair_duration = duration / ((end_index - start_index + 1) * 2)  # Duration for each pair
    
    for i in range(start_index, end_index + 1):
        while total_duration < duration and total_duration + pair_duration <= duration:
            try:
                # Load video clip
                ...
                total_duration += 2 * pair_duration  # Each pair adds twice the duration
